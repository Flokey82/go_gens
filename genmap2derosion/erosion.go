...
type World struct {
	...
	heightmap   [worldsize * worldsize]float64 // Flat Array containing height information
	sediment    [worldsize * worldsize]float64 // Sediment storage (stores information on eroded / deposited sediment)
	waterpath   [worldsize * worldsize]float64 // Water Path Storage (Rivers)
	waterpool   [worldsize * worldsize]float64 // Water Pool Storage (Lakes / Ponds)
	waterdrains [worldsize * worldsize]int     // Water Pool drainage points

	// Flux related information (experimental)
	fluxwaterpool [worldsize * worldsize]float64 // (TEMP Flux) Water Pool Storage (Lakes / Ponds)
...
func NewWorld() *World {
	w := &World{
		drainage: 0.01,
		scale:    40.0,
		dim:      vectors.NewIVec2(worldsize, worldsize),
		fdim:     vectors.NewVec2(float64(worldsize), float64(worldsize)),
	}
...
func (w *World) storeGifFrame(h, waterPath, waterPool []float64) {
	const width, height = worldsize, worldsize
